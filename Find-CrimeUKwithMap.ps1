...
.EXAMPLE
   Find-CrimeUK -LocationName Bridgemary -Year 2020 -Month 01 -MapZoom 10
.NOTES
   ...
   Process {
      ...
      try {
         ...
         foreach($SelectedProp in $SelectedProps){ 
            ...
            Start-Process microsoft-edge:"https://www.openstreetmap.org/#map=$($MapZoom)/$($SelectedProp.Latitude)/$($SelectedProp.Longitude)" -Verbose -ErrorAction Stop
